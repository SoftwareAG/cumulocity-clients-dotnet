...
namespace Com.Cumulocity.Client.Api 
{
	...
	public class CurrentUserApi : AdaptableApi, ICurrentUserApi
	{
		...
		public async Task<CurrentUser?> GetCurrentUser(CancellationToken cToken = default) 
		{
			...
			return await JsonSerializer.DeserializeAsync<CurrentUser?>(responseStream, cancellationToken: cToken);
		}
		...
		public async Task<CurrentUserTotpSecret?> GenerateTfaSecret(CancellationToken cToken = default) 
		{
			...
			response.EnsureSuccessStatusCode();
			using var responseStream = await response.Content.ReadAsStreamAsync();
			return await JsonSerializer.DeserializeAsync<CurrentUserTotpSecret?>(responseStream, cancellationToken: cToken);
		}
		...
		public async Task<CurrentUserTotpSecretActivity?> GetTfaState(CancellationToken cToken = default) 
		{
			...
			response.EnsureSuccessStatusCode();
			using var responseStream = await response.Content.ReadAsStreamAsync();
			return await JsonSerializer.DeserializeAsync<CurrentUserTotpSecretActivity?>(responseStream, cancellationToken: cToken);
		}
