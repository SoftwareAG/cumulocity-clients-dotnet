...
public sealed class LoginOptionsApi : ILoginOptionsApi
{
	...
	public async Task<LoginOptionCollection?> GetLoginOptions(bool? management = null, string? tenantId = null, CancellationToken cToken = default) 
	{
		...
		return await JsonSerializerWrapper.DeserializeAsync<LoginOptionCollection?>(responseStream, cancellationToken: cToken).ConfigureAwait(false);
	}
	...
	public async Task<AuthConfig?> CreateLoginOption(AuthConfig body, CancellationToken cToken = default) 
	{
		...
		await response.EnsureSuccessStatusCodeWithContentInfo().ConfigureAwait(false);
		await using var responseStream = await response.Content.ReadAsStreamAsync().ConfigureAwait(false);
		return await JsonSerializerWrapper.DeserializeAsync<AuthConfig?>(responseStream, cancellationToken: cToken).ConfigureAwait(false);
	}
	...
	/// <inheritdoc />
	public async Task<string?> DeleteLoginOption(string typeOrId, CancellationToken cToken = default) 
	{
		string resourcePath = $"/tenant/loginOptions/{HttpUtility.UrlPathEncode(typeOrId.GetStringValue())}";
		var uriBuilder = new UriBuilder(new Uri(_httpClient.BaseAddress ?? new Uri(resourcePath), resourcePath));
		using var request = new HttpRequestMessage 
	...
	public async Task<AuthConfig?> UpdateLoginOptionAccess(AuthConfigAccess body, string typeOrId, string? targetTenant = null, CancellationToken cToken = default) 
	{
		...
		string resourcePath = $"/tenant/loginOptions/{HttpUtility.UrlPathEncode(typeOrId.GetStringValue())}/restrict";
		var uriBuilder = new UriBuilder(new Uri(_httpClient.BaseAddress ?? new Uri(resourcePath), resourcePath));
		var queryString = HttpUtility.ParseQueryString(uriBuilder.Query);
