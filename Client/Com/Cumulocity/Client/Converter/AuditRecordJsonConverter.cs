...
public class AuditRecordJsonConverter<T> : JsonConverter<T> where T : AuditRecord
{
	...
	public override T? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
	{
		...
		using (var jsonDocument = JsonDocument.ParseValue(ref reader))
		{
			...
			while (objectEnumerator.MoveNext())
			{
				...
				else if (AuditRecord.Serialization.AdditionalPropertyClasses.ContainsKey(current.Name))
				{
					var type = AuditRecord.Serialization.AdditionalPropertyClasses[current.Name];
