//
// CustomPropertiesJsonConverter.cs
...
public class CustomPropertiesJsonConverter<T> : JsonConverter<T> where T : CustomProperties
{
	...
	public override T? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
	{
		...
		using (var jsonDocument = JsonDocument.ParseValue(ref reader))
		{
			...
			while (objectEnumerator.MoveNext())
			{
				...
				else if (CustomProperties.Serialization.AdditionalPropertyClasses.ContainsKey(current.Name))
				{
					var type = CustomProperties.Serialization.AdditionalPropertyClasses[current.Name];
