...
{
	public class MeasurementJsonConverter<T> : JsonConverter<T> where T : Measurement
	{
		...
		public override T? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
		{
			...
			using (var jsonDocument = JsonDocument.ParseValue(ref reader))
			{
				...
				while (objectEnumerator.MoveNext())
				{
					...
					else if (Measurement.Serialization.AdditionalPropertyClasses.ContainsKey(current.Name))
					{
						var type = Measurement.Serialization.AdditionalPropertyClasses[current.Name];
