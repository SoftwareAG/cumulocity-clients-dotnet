...
{
	public class OperationJsonConverter<T> : JsonConverter<T> where T : Operation
	{
		...
		public override T? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
		{
			...
			using (var jsonDocument = JsonDocument.ParseValue(ref reader))
			{
				...
				while (objectEnumerator.MoveNext())
				{
					...
					else if (Operation.Serialization.AdditionalPropertyClasses.ContainsKey(current.Name))
					{
						var type = Operation.Serialization.AdditionalPropertyClasses[current.Name];
